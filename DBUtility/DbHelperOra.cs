...
namespace DBUtility
{
    ...
    {
        public static string connectionString = Common.DES.DESDecrypt(File.ReadAllText(@"config.ini"), "WENWARDH");
        public DbHelperOra()
        {
        }
        #region 公用方法
        ...
        public static int ExecuteSql(string SQLString)
        {
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = new OracleCommand(SQLString, connection))
                {
                    try
                    {
                        //Common.DataProces.writelog(SQLString);
                        connection.Open();
        ...
        public static DataSet Query(string SQLString)
        {
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    {
                        //设置查找列名称
                        if (col.ColumnName.StartsWith("SSSS"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(colName))
                            {
                                ...
                                foreach (DataRow row in ds.Tables[0].Rows)
                                {
                                    if (row[colName] != null)
                                    {
                                        row[colName] = Common.DES.ssssdecrypt(row[colName].ToString());
                                    }
                                }
                            }
                            continue;
